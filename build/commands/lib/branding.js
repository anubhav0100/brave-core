...
const update = () => {
  ...
  for (const branding of ['brave', 'brave_origin', 'ai_automation_browser']) {
  ...
  // Use brave_origin/ai_automation_browser logos when their respective
  // branding flags are true.
  const productLogoSource = config.isBraveOriginBranded
    ? 'brave_origin'
    : config.isAiAutomationBrowserBranded
  ...
  for (const scale of ['default_100_percent', 'default_200_percent']) {
    ...
    // Replace the New Tab Page's tab favicon (IDR_NTP_FAVICON), which
    // otherwise stays a hardcoded Brave icon regardless of branding flag.
    fileMap.add([
      path.join(
        braveAppDir,
        'theme',
      ...
      path.join(chromeAppDir, 'theme', scale, 'common', 'favicon_ntp.png'),
    ])
  }
  for (const branding of ['brave', 'brave_origin', 'ai_automation_browser']) {
    fileMap.add([
